...
from src.unet3d.buildingblocks import DoubleConv, ResNetBlock, ResNetBlockSE, \
    create_decoders, create_encoders, InterpolateUpsampling
from src.unet3d.utils import get_class, number_of_features_per_level
...
class AttUNet_AP(nn.Module):
    def __init__(self,img_ch=3,output_ch=1):
        super(AttUNet_AP,self).__init__()
        ...
        self.Up5 = up_conv(ch_in=1024,ch_out=512)
        self.Att5 = Attention_block(F_g=1,F_l=512,F_int=256)
        self.Up_conv5 = conv_block(ch_in=1024, ch_out=512)

        self.Up4 = up_conv(ch_in=512,ch_out=256)
        self.Att4 = Attention_block(F_g=1,F_l=256,F_int=128)
        self.Up_conv4 = conv_block(ch_in=512, ch_out=256)
        
        self.Up3 = up_conv(ch_in=256,ch_out=128)
        self.Att3 = Attention_block(F_g=1,F_l=128,F_int=64)
        self.Up_conv3 = conv_block(ch_in=256, ch_out=128)
        
        self.Up2 = up_conv(ch_in=128,ch_out=64)
        self.Att2 = Attention_block(F_g=1,F_l=64,F_int=32)
        self.Up_conv2 = conv_block(ch_in=128, ch_out=64)
    ...
    def forward(self,x):
        ...
        # decoding + concat path
        d5 = self.Up5(x5)
        g5 = F.interpolate(x[:,:,2,:,:],size=d5.size()[2:], mode='nearest')
        x4 = self.Att5(g=g5,x=x4) # x4 = self.Att5(g=d5,x=x4)
        d5 = torch.cat((x4,d5),dim=1)        
        d5 = self.Up_conv5(d5)
        
        d4 = self.Up4(d5)
        g4 = F.interpolate(x[:,:,2,:,:],size=d4.size()[2:], mode='nearest')
        x3 = self.Att4(g=g4,x=x3)
        d4 = torch.cat((x3,d4),dim=1)
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4)
        g3 = F.interpolate(x[:,:,2,:,:],size=d3.size()[2:], mode='nearest')
        x2 = self.Att3(g=g3,x=x2)
        d3 = torch.cat((x2,d3),dim=1)
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3)
        g2 = F.interpolate(x[:,:,2,:,:],size=d2.size()[2:], mode='nearest')
        x1 = self.Att2(g=g2,x=x1)
        d2 = torch.cat((x1,d2),dim=1)
        d2 = self.Up_conv2(d2)
...
class SegNet(nn.Module):
    ...
    def __init__(self, in_chn=3, out_chn=32, BN_momentum=0.5):
        ...
        # self.Conv_1x1 = nn.Conv2d(32,self.out_chn,kernel_size=1,stride=1,padding=0)
        self.final_activation = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        x = self.final_activation(x) # F.softmax(x, dim=1)
